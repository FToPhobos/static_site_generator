...
def generate_page(from_path, template_path, dest_path, basepath):
    ...
    template_data = template_data.replace('href="/', 'href="' + basepath)
    template_data = template_data.replace('src="/', 'src="' + basepath)
...
def generate_page_recursive(from_path, template_path, dest_path, basepath):
    for path in os.listdir(from_path):
        ...
        if os.path.isdir(full_path) == True:
            generate_page_recursive(full_path, template_path, full_dest_path, basepath)
        else:
            if path.endswith(".md"):
                final = full_dest_path.replace(".md", ".html")
                generate_page(full_path, template_path, final, basepath)
